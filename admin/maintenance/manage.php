...
<div class="card card-outline card-info">
    ...
    <div class="card-body">
        <form action="" id="package-form">
            ...
            <div class="form-group">
                <label for="tour_location" class="control-label">Địa chỉ</label>
            ...
            <div class="form-group">
                ...
                <select name="status" id="status" class="custom-select selevt">
                    <option value="1" <?php echo isset($status) && $status == 1 ? 'selected' : '' ?>>Đang hoạt động</option>
                    <option value="0" <?php echo isset($status) && $status == 0 ? 'selected' : '' ?>>Ngừng hoạt động</option>
                </select>
            </div>
            <div class="form-group">
...
<script>
    ...
    function delete_img($path) {
        ...
        $.ajax({
            ...
            error: err => {
                console.log(err)
                alert_toast("Có lỗi xảy ra khi xóa hình ảnh", "error");
                end_loader()
            },
            success: function(resp) {
                ...
                if (typeof resp == 'object' && resp.status == 'success') {
                    ...
                    alert_toast("Đã xóa hình ảnh", "success");
                } else {
                    console.log(resp)
                    alert_toast("Có lỗi xảy ra khi xóa hình ảnh", "error");
                }
    ...
    $(document).ready(function() {
        $('.rem_img').click(function() {
            _conf("Bạn có chắc muốn xóa hình ảnh này vĩnh viễn?", 'delete_img', ["'" + $(this).attr('data-path') + "'"])
        })
